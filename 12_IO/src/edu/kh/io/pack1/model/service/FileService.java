...
public class FileService {
	 ...
	 * -경로 표기 방법(하위폴더, 파일 표시)
	 * 	
	 * 백슬래쉬(\, Windows)    ->C:\workspace\02_JAVA
	 * 슬래쉬  (/, Linux, Mac) ->C:/workspace/02_JAVA
	 * 
	 * 
	 * -Java 언어의 특징 : 플랫폼(OS)에 독립적
	 * ->OS에 관계없이 똑같은 코드 작성이 가능하다
	 * 
	 * -> 이 특징을 유지하기 위해 자바에서는 경로 표시에서 슬래쉬, 백슬래쉬 둘 다 호환이 가능하다
	...
	public void method1() {
		...
		//C;/io_test/20240222 폴더를 지정하는 파일 객체 생성하기
		...
		if(!directory.exists()) { //폴더가 존재하지 않으면
			...
			//폴더 모두 생성
			directory.mkdirs();
			System.out.println(directory.getName()); //제일 마지막에 있는 파일이나 폴더명 반환 20240222
	...
	public void method2() {
		...
		try {
			...
			e.printStackTrace(); //예외 정보 + 예외가 발생한 위치까지의 메서드 추적해줌
	...
	public void method3() {
		...
		for(File f : files) {
			//String File.toString() : 지정된 파일과 디렉토리의 절대경로를 String으로 보여줌
			...
			//	매개변수 Long 값을 지정된 패턴 형식으로 변화하여 문자열 형태로 반환
